...
const BookkeepingForm = () => {
  // Full form state (sNo, fileName, reviewer, fees removed; urgentReason added)
  ...
  // input change handler (handles period formatting)
  ...
  // Duplicate checker (normalizes headers automatically)
  const isDuplicate = (rows, data) => {
    if (!rows || rows.length === 0) return false;

    return rows.some(r => {
      const cNo   = (r["Company Number"] || r.companyNo || "").toString().trim().toLowerCase();
      const cName = (r["Company Name"]   || r.companyName || "").toString().trim().toLowerCase();
      const utr   = (r["UTR"]            || r.companyUTR || "").toString().trim().toLowerCase();
      const work  = (r["Work Done"]      || r.workDone || "").toString().trim().toLowerCase();
      const period= (r["Period"]         || r.period || "").toString().trim().toLowerCase();

      return (
        cNo   === (data.companyNo || "").toString().trim().toLowerCase() &&
        cName === (data.companyName || "").toString().trim().toLowerCase() &&
        utr   === (data.companyUTR || "").toString().trim().toLowerCase() &&
        work  === (data.workDone || "").toString().trim().toLowerCase() &&
        period=== (data.period || "").toString().trim().toLowerCase()
      );
    });
  };
  ...
  const validateForm = () => {
    ...
    // If urgent is checked, urgentReason is required
    if (formData.urgent && !formData.urgentReason) {
      missing.push({ field: 'urgentReason', label: 'Urgent Reason' });
    }
  ...
  // Helper: register or update a reminder on the proxy
const registerReminderForRecord = async (submissionData) => {
  try {
    const payload = {
      companyNo: submissionData.companyNo || '',
      companyName: submissionData.companyName || '',
      bookkeeper: submissionData.bookkeeper || '',
      allocatedTo: submissionData.fileAllocatedTo || '',
      status: submissionData.status || '',
      period: submissionData.period || '',
      reference: submissionData.clientReference || '',
      timestamp: submissionData.timestamp || new Date().toISOString()
    };

    const resp = await fetch(`${PROXY_URL}/api/reminders`, {
      method: 'POST',
    ...
    if (!resp.ok) {
      ...
      console.warn('Failed to register reminder', resp.status, txt);
      return false;
    }
    return true;
  } catch (err) {
    console.error('Error registering reminder:', err);
    return false;
  }
};

// Helper: mark reminder complete (stop reminders)
const completeReminderForRecord = async (companyNoOrId) => {
  try {
    const resp = await fetch(`${PROXY_URL}/api/reminders/complete`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ companyNo: companyNoOrId })
    });
    if (!resp.ok) {
      const t = await resp.text().catch(()=>null);
      console.warn('Failed to complete reminder', resp.status, t);
      return false;
    }
    return true;
  } catch (err) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then(payload => {
        ...
        // candidate header names (no reviewer/fileName here)
        ...
        // Make companyNames array (ensure variable name is correct)
...
const handleSubmit = async () => {
  ...
  // 2) duplicate check BEFORE sending
  if (isDuplicate(csvRows, formData)) {
    alert("Duplicate entry detected. This record already exists.");
    setSubmitStatus({ type: 'error', message: "Duplicate entry detected — not submitted." });
    return;
  }

  // 3) prepare payload (ensure numeric fields are numbers)
  ...
  try {
    ...
    if (appsResp && appsResp.success === true) {
      ...
      // Auto-register / update reminders: only if assigned and not Completed
      try {
        if ((submissionData.bookkeeper || submissionData.fileAllocatedTo) && (submissionData.status !== 'Completed')) {
          // await so we can log failures; change to fire-and-forget if you prefer
          const ok = await registerReminderForRecord(submissionData);
          if (!ok) console.warn('Reminder registration failed for', submissionData.companyNo || submissionData.companyName);
        } else if (submissionData.status === 'Completed') {
          const ok = await completeReminderForRecord(submissionData.companyNo || submissionData.companyName);
          if (!ok) console.warn('Failed to mark reminder complete for', submissionData.companyNo || submissionData.companyName);
        }
      } catch (remErr) {
        console.error('Reminder handling error', remErr);
      }

    } else if (response.ok && !appsResp) {
      // Some proxies return 200 without JSON body - treat as success
    ...
    <div>
      ...
      {formData.urgent && (
        <div style={{ marginTop: '12px' }}>
          <label style={{ display: 'block', fontSize: 14, fontWeight: 500, color: '#374151', marginBottom: 8 }}>Reason for Urgency *</label>
          <input type="text" name="urgentReason" value={formData.urgentReason} onChange={handleInputChange} placeholder="Enter reason for urgency" style={{ width: '100%', padding: '12px', border: '1px solid #d1d5db', borderRadius: '8px' }} />
        </div>
      )}
    </div>
      ...
      <input type="text" name="month" value={formData.month} readOnly style={{ width: '100%', padding: '12px', border: '1px solid #d1d5db', borderRadius: '8px', backgroundColor: '#f9fafb', color: '#6b7280' }} />
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: 14, fontWeight: 500, color: '#374151', marginBottom: 8 }}>File Type</label>
                    <select
                      name="fileType"
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', marginBottom: '24px' }}>
            ...
            <div style={{ fontSize: 14, color: '#6b7280', lineHeight: 1.6 }}>
              <div style={{ marginBottom: 12 }}>
                ...
                • Example headers: month, clientReference, companyNo, companyName, companyUTR, bookkeeper, totalEntries, notes, etc.
              </div>
